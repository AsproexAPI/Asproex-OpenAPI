package io.asproex.api.client.security;
...
public class AuthenticationInterceptor implements Interceptor {
    ...
    public Response intercept(Chain chain) throws IOException {
        ...
        boolean isApiKeyRequired = original.header(AsproexConstants.ENDPOINT_SECURITY_TYPE_APIKEY) != null;
        boolean isSignatureRequired = original.header(AsproexConstants.ENDPOINT_SECURITY_TYPE_SIGNED) != null;
        newRequestBuilder.removeHeader(AsproexConstants.ENDPOINT_SECURITY_TYPE_APIKEY)
                .removeHeader(AsproexConstants.ENDPOINT_SECURITY_TYPE_SIGNED);

        // Endpoint requires sending a valid API-KEY
        if (isApiKeyRequired || isSignatureRequired) {
            newRequestBuilder.addHeader(AsproexConstants.API_KEY_HEADER, apiKey);
        }
